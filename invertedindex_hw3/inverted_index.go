...
type InvertedIndex struct {
	...
	kgrams map[string]map[string]struct{}
	k      int
}
...
func NewInvertedIndexWithLSM(maxSize int, dir string) *InvertedIndex {
	return &InvertedIndex{
		...
		kgrams: make(map[string]map[string]struct{}),
		k:      3,
	}
}
...
func (idx *InvertedIndex) addTerm(term string) {
	if term == "" {
		return
	}
	...
	for _, gram := range termKgrams(term, idx.k) {
		if idx.kgrams[gram] == nil {
			idx.kgrams[gram] = make(map[string]struct{})
		}
		idx.kgrams[gram][term] = struct{}{}
	}
}

func termKgrams(term string, k int) []string {
	if term == "" || k <= 0 {
		return nil
	}
	padded := "$" + term + "$"
